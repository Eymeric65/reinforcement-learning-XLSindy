    ...
    try:
        ...
        for iteration in range(1,args.num_iterations +1):
            ...
            for step in range(0,args.num_steps):
                ...
                ### Some issue with this logger (slowdown everything) 

                ## Added np.array(..) wrapper for jax array 
                ...
                #print(infos["reward_info"]["goal_state"])
                ## Fundamentally broken for the moment
                # if "final_info" in infos:
                #     for info in infos["final_info"]:
                #         if info and "episode" in info:
                #             print(f"global_step={global_step}, episodic_return={info['episode']['r']}")
                #             for r in np.array(info["episode"]["r"]):
                #                 writer.add_scalar("charts/episodic_return", r, global_step)
                #             for r in np.array(info["episode"]["l"]):
                #                 writer.add_scalar("charts/episodic_length", r, global_step)
            ...
            writer.add_scalar("losses/mean_reward", rewards.mean(), global_step)
            writer.add_scalar("losses/max_reward", rewards.max(), global_step)
            writer.add_scalar("losses/min_reward", rewards.min(), global_step)
